...
if request.method == 'POST':
    cursor.execute(
        "INSERT INTO employees (name, salary, user_id) VALUES (%s, %s, %s)",
...
@app.route('/attendance/[int:id](int:id)', methods=['GET', 'POST'])
def attendance(id):
if session.get('role') != "admin":
return "Access denied"
...
if request.method == 'POST':
    ...
    cursor.execute(
        "INSERT INTO attendance (employee_id, user_id, date, status) VALUES (%s, %s, %s, %s)",
...
@app.route('/payslip/[int:id](int:id)')
def payslip(id):
conn = get_db_connection()
cursor = conn.cursor()
